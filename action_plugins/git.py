...
class ActionModule(ActionBase):
    ...
    def _parse_repo_uri(self, repo):
        repo_parsed = urlparse(repo)
        if repo_parsed.path.endswith('.git'):
            repo_parsed.path = repo_parsed.path.replace('.git', '')
        return "https://{hostname}/{path}".format(
            hostname=repo_parsed.hostname,
            path=repo_parsed.path)
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        uri_parsed = self._parse_repo_uri(git_args['repo'])
        ...
        cmpt_url = "%s/commit/%s" % (uri_parsed, _commit_id)
