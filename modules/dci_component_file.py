...
options:
  ...
  dest:
    required: false
    description: Where to store the downloaded file. If file_id is defined, only the given file will be download, otherwise, all the files are downloaded.
  src:
    required: false
...
def check_http_code(module, response, expectation):
    if str(response.status_code) not in expectation:
        module.fail_json(msg='Unexpected error code (%s): %s' % (
            response.status_code, res.text))


def get_file_ids(module):
...
def main():
    ...
    # delete
    if module.params.get('stats') == 'absent':
        for file_id in get_file_ids(module):
    ...
    # download (and extract)
        ...
        for file_id in get_file_ids(module):
            target = module.params['cache'] + '/' + file_id + '.tar'
            ...
            if module.params.get('extract'):
                with tarfile.open(name=target, mode='r:') as tf:
                    tf.extractall(path=module.params['dest'])
