...
def test_copy_success(capsys, tmpdir):
    ...
    if 'Aborting, target uses selinux but python bindings' not in out:
        assert 'Copying to file' in out


def test_copy_failure(capsys):
    run_task(dict(action=dict(module='copy', args='src=/etc/aliases dest=/proc/aliases')))
    expectation = 'Copying to file: None (changed: False)\n\n'
    assert capsys.readouterr().out == expectation
...
def test_package_failure(capsys):
    ...
    out = capsys.readouterr().out
    if 'The Python 2 bindings for rpm are needed for this module.' not in out:
        assert out == ''
