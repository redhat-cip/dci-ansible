...
def test_copy_success(capsys):
    run_task(dict(action=dict(module='copy', args='src=/etc/fstab dest=/tmp/fstab_')))
...
def test_copy_failure(capsys):
    run_task(dict(action=dict(module='copy', args='src=/etc/fstab dest=/proc/fstab')))
    assert capsys.readouterr().out == ''


def test_dci_topic_failure_bad_params(capsys):
    run_task(dict(action=dict(module='dci_topic', args='named=bob')))
    # TODO(Gonéri): Wrong output
    expectation = ("{'_ansible_parsed': True, '_ansible_no_log': False, "
                   "'changed': False, u'invocation': {u'module_args': "
                   "{u'named': u'bob'}}, u'msg': u'Unsupported parameters "
                   "for (dci_topic) module: named Supported parameters "
                   "include: active, component_types, dci_api_secret, "
                   "dci_client_id, dci_cs_url, dci_login, dci_password, "
                   "embed, id, label, name, next_topic, product_id, state, "
                   "team_ids, where'}\n\n")
    assert capsys.readouterr().out == expectation
...
def test_file_success(capsys):
    run_task(dict(action=dict(module='file', args='path=/etc/fstab state=present')))
    assert capsys.readouterr().out == ''
